...
function calculateSum(n) {
  let sum = 0; // Ініціалізуємо змінну sum зі значенням 0
  for (let i = 1; i <= n; i++) {
    sum += i; // Додаємо поточне значення i до змінної sum
  }
...
function isPrimeNumber(num) {
  if (num <= 1) {
    return false; // Якщо число менше або дорівнює 1, воно не є простим
  }
  ...
  for (let i = 2; i < num; i++) {
    ...
    if (num % i === 0) {
      return false; // Якщо num ділиться націло на i, воно не є простим
    }
  }

  return true; // Якщо під час циклу не знайдено дільників, num є простим
}
...
function memoizedArea() {
  ...
  return function (side) {
    ...
    if (side === prevSide) {
      ...
      // Зберігаємо результат та аргумент для майбутнього використання
      prevSide = side;
      prevResult = result;
      // Повертаємо площу
      return result;
